...
## V0.2.4 Premade Playtest Edition
- Added four automatically seeded Level 4 Trainers.
- All four Trainers use different classes.
- Each has one randomly selected Kanto starter plus three additional Pokémon.
- All included Pokémon are Pokédex No. 100 or earlier.
- Every included Pokémon is Level 5–10.
- Premade Pokémon receive their normal level-appropriate natural moves automatically.
- Premade edition uses a separate local save key so it will not replace normal V0.2.x save data.
- Retains Pokémon Center healing, encounter-limited Trainer features, caught-Pokémon current-HP entry, natural move progression, ASI selection, and layout fixes from the current V0.2.4 line.
