...
function pokemonSprite(sp,p=null,cls='pokemon-sprite'){const sub=!!p?.substituteActive;const src=sub?'assets/pokemon/substitute.png':spritePath(sp.id);return `<img class="${cls}${sub?' substitute-sprite':''}" src="${src}" alt="${escapeHTML(sub?'Substitute':sp.name)}" loading="lazy">`;}
...
function renderSideNav(tr){return `<aside class="side-nav">
  <div class="brand"><div class="mini-orb"></div><div><div class="brand-title">Pokémon d20</div><div class="brand-sub">Player Companion • V0.2.5 Premade</div></div></div>
