...
function ensurePokemonMoveProgress(p,{initializeActive=false,migrateLegacy=false}={}){
  ...
  if(p.substituteActive==null)p.substituteActive=false;
  if(p.shiny==null)p.shiny=false;
...
function spritePath(speciesId){return `assets/pokemon/${String(Number(speciesId)).padStart(4,'0')}.png`;}
function shinySpritePath(speciesId){return `assets/pokemon/shiny/${String(Number(speciesId)).padStart(4,'0')}.png`;}
function pokemonSprite(sp,p=null,cls='pokemon-sprite'){const sub=!!p?.substituteActive,shiny=!!p?.shiny;const src=sub?'assets/pokemon/substitute.png':(shiny?shinySpritePath(sp.id):spritePath(sp.id));const label=sub?'Substitute':`${shiny?'Shiny ':''}${sp.name}`;return `<img class="${cls}${sub?' substitute-sprite':''}${shiny&&!sub?' shiny-sprite':''}" src="${src}" alt="${escapeHTML(label)}" loading="lazy">`; }
...
function renderSideNav(tr){return `<aside class="side-nav">
  <div class="brand"><div class="mini-orb"></div><div><div class="brand-title">Pokémon d20</div><div class="brand-sub">Player Companion • V0.2.7 Premade</div></div></div>
...
function renderParty(tr){
  ...
  <div class="card" style="margin-top:18px"><div class="card-head"><div><div class="card-title">PC Storage</div><div class="card-sub">${storage.length} Pokémon stored • HP, fainting, moves, Battle Marks and held items are preserved.</div></div></div>${storage.length?`<div class="dex-grid">${storage.map(p=>{const sp=speciesById(p.speciesId);return `<button class="dex-card" data-pokemon="${p.id}"><div class="dex-sprite">${pokemonSprite(sp,p)}</div><div class="dex-no">${sp.number} • LV ${p.level}</div><div class="dex-name">${escapeHTML(p.nickname||sp.name)}</div><div>${typeBadges(sp.types)}${p.shiny?'<span class="tag">SHINY</span>':''}${p.substituteActive?'<span class="tag">SUBSTITUTE</span>':''}</div><div class="dex-state">${p.currentHp<=0?'FAINTED':`${p.currentHp}/${p.maxHp} HP`} • ${escapeHTML(p.heldItemId?itemById(p.heldItemId)?.name||'Held item':'No held item')}</div></button>`}).join('')}</div>`:`<div class="empty-state"><div class="empty-icon">▦</div><strong>Storage is empty</strong><p>Captured Pokémon go here when your party is full.</p></div>`}</div>`;
}
...
function renderCreator(){
  if(!ui.creatorDraft) ui.creatorDraft={name:'',className:'Trainer',background:'Academy Student',abilityScores:{Strength:15,Dexterity:14,Constitution:13,Intelligence:12,Wisdom:10,Charisma:8},skills:[],starterId:1,starterLevel:1,starterNickname:'',starterShiny:false};
...
function bindCreator(){
  ...
  $('#cr-starter-shiny')?.addEventListener('change',e=>{ui.creatorDraft.starterShiny=e.target.checked;render()});
...
function createTrainerFromDraft(){
  ...
  const p=createPokemonInstance(d.starterId,d.starterLevel,d.starterNickname);p.shiny=!!d.starterShiny;tr.pokemon.push(p);tr.partyIds.push(p.id);tr.activePokemonId=p.id;tr.pokedex[d.starterId]='caught';
...
function openPokemonSheet(pid){
  ...
  <div class="grid grid-2" style="margin-top:14px"><div class="card soft"><div class="card-head"><div><div class="card-title">Active Moves</div><div class="card-sub">Four active move slots. Tap Manage to use the Relearnable Move Pool.</div></div><button class="ghost-btn btn-sm" data-pk-action="moves">Manage</button></div>${p.substituteActive?`<div class="substitute-banner"><div><strong>Substitute active</strong><div class="tiny muted">The Pokémon sprite is replaced by the Substitute until you mark it broken or finished.</div></div><button class="danger-btn btn-sm" data-end-substitute>Break / End</button></div>`:''}<div class="move-slot-grid">${[0,1,2,3].map(i=>{const name=p.activeMoves[i];const mv=name?moveByName(name):null;return name&&mv?`<div class="move-slot"><div>${typeBadges([mv.type])}</div><div class="move-slot-name">${escapeHTML(name)}</div><div class="move-slot-rule">${escapeHTML(mv.rules)}</div>${name==='Substitute'?`<button class="ghost-btn btn-sm" style="margin-top:10px" data-use-substitute ${p.substituteActive?'disabled':''}>${p.substituteActive?'Substitute active':'Use Substitute'}</button>`:''}</div>`:`<div class="move-slot empty">Empty move slot</div>`}).join('')}</div></div><div class="card soft"><div class="card-title">Held Item</div><div class="card-sub">${held?escapeHTML(held.effect):'No held item equipped.'}</div><div style="margin-top:10px">${held?`<strong>${escapeHTML(held.name)}</strong> <span class="tag">${escapeHTML(held.mode||'Held')}</span>`:'<span class="muted">Empty slot</span>'}</div><button class="ghost-btn btn-sm" style="margin-top:12px" data-pk-action="held">Manage held item</button><div class="rule-note warning-note" style="margin-top:12px"><strong>No dice automation.</strong> When you use Substitute, record its HP cost and damage at the table; this button only changes the displayed sprite/state.</div></div></div>
  ...
  $('[data-use-substitute]',m)?.addEventListener('click',()=>{p.substituteActive=true;saveState();closeModal();render();openPokemonSheet(pid)});
  $('[data-end-substitute]',m)?.addEventListener('click',()=>{p.substituteActive=false;saveState();closeModal();render();openPokemonSheet(pid)});
...
function openAddPokemon(prefillSpeciesId=null){
  ...
  const body=`<div class="form-grid"><div class="field"><label>Species</label><select id="add-species">${DATA.pokemon.map(p=>`<option value="${p.id}" ${p.id===initialSid?'selected':''}>${p.number} ${escapeHTML(p.name)}</option>`).join('')}</select></div><div class="field"><label>Level</label><input id="add-level" type="number" min="1" max="20" value="1"></div><div class="field"><label>Current HP</label><input id="add-current-hp" type="number" min="0" max="${initialMax}" placeholder="Leave blank for full HP"><div class="tiny muted" id="add-max-hp">Maximum HP at this level: ${initialMax}</div></div><div class="field"><label>Nickname</label><input id="add-nickname" placeholder="Optional"></div><div class="field"><label>Variant</label><label class="check-card" style="cursor:pointer"><input id="add-shiny" type="checkbox"><span>Shiny Pokémon</span></label></div></div><div class="rule-note info-note" style="margin-top:16px">Use this after a successful capture, receiving a Pokémon, or when the GM instructs you to add one. For a captured Pokémon, enter the HP it had remaining when caught; capture does not heal it. Leave Current HP blank only when the Pokémon should enter at full health. Natural moves up to the chosen level are learned automatically.</div><button class="primary-btn" style="margin-top:18px" id="add-confirm">Add Pokémon</button>`;
...
loadData().then(()=>{
  ...
  let migrated=false;
  for(const tr of state.trainers){const beforeResources=JSON.stringify(tr.encounterResources||{});ensureEncounterResources(tr);if(JSON.stringify(tr.encounterResources||{})!==beforeResources)migrated=true;for(const p of (tr.pokemon||[])){const before=JSON.stringify([p.activeMoves,p.relearnableMoves,p.learnedTMMoves,p.moveProgressionMigrated,p.shiny]);ensurePokemonMoveProgress(p,{initializeActive:true,migrateLegacy:true});if(JSON.stringify([p.activeMoves,p.relearnableMoves,p.learnedTMMoves,p.moveProgressionMigrated,p.shiny])!==before)migrated=true}}
